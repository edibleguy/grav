...
class __TwigTemplate_5c59bf177eca07c31413237362ee55004810cf838d528c39bbb7801c86be1898 extends Twig_Template
{
...
{% macro displayItem(item, menu, context, dropdown_type) %}
    ...
    {% if item.visible and item.enabled and enabled|default(1) %}
        {% set title = item.icon_only or item.link_title ? ' title=\"' ~ item.link_title|default(item.title) ~ '\"' %}
        {% set active = menu.isActive(item) ? ' active' %}
        {% set dropdown = item.level == 1 ? ' g-' ~ item.getDropdown() %}
        {% set parent = item.children ? ' g-parent' %}
        {% set target = item.target != '_self' ? ' target=\"' ~ item.target ~ '\"' %}
        {% set rel = item.rel ? ' rel=\"' ~ item.rel ~ '\"' %}

        <li class=\"g-menu-item g-menu-item-type-{{ item.type }} g-menu-item-{{ item.id }}{% if not item.dropdown_hide %}{{ parent|raw }}{% endif %}{{ active|raw }}{{ dropdown|raw }} {% if item.url and item.children %}{% if not item.dropdown_hide %}g-menu-item-link-parent{% endif %}{% endif %} {{ item.class|default('') }}\"
                {{- SELF.getCustomWidth(item, menu, 'item', dropdown) }}
                {%- if context.particle.renderTitles|default(0) %} title=\"{{ item.title }}\"{% endif %}>
            {% if item.url %}<a class=\"g-menu-item-container{{ item.anchor_class ? ' ' ~ item.anchor_class }}\" href=\"{{ item.url }}{{ item.hash }}\"{{ (title ~ target ~ rel)|raw }}>
...
{% endif %}
", "@particles/menu.html.twig", "/var/www/html/grav/user/plugins/gantry5/engines/nucleus/particles/menu.html.twig");
    }
}
